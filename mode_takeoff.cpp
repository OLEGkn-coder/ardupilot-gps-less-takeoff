...
void ModeTakeoff::update()
{
    ...
    if (plane.flight_stage == AP_FixedWing::FlightStage::TAKEOFF && plane.steer_state.hold_course_cd == -1 // This is the enter-once flag.
         ...
         start_loc.get_distance(plane.current_loc) >= dist))
    {
        ...
        // Переводимо борт в режим CIRCLE після досягнення необхідньої висоти
        plane.set_mode(Mode::Number::CIRCLE, ModeReason::TAKEOFF_COMPLETE);
    }

...
bool Plane::auto_takeoff_check(void)
{
    ...
    // Перевіряємо чи присутній датчик повітряної швидкості та чи задано EKF origin
    Location origin;
    bool have_origin = ahrs.get_origin(origin);
    bool have_air_speed = ahrs.using_airspeed_sensor();

    // Check for bad GPS
    if (gps.status() < AP_GPS_FixType::FIX_3D && !(have_origin && have_air_speed))
    {
        // no auto takeoff without GPS lock
        return false;
    }
    // Якщо ж або GPS або наша зв'зка то йдемо далі, якщо нема нічого повертаємо false
    ...
    // Беремо швидкість або повітряну або з GPS
    float current_speed = 0.0f;
    if (ahrs.using_airspeed_sensor())
    {
        ahrs.airspeed_estimate(&current_speed);
    }
    else
    {
        current_speed = gps.ground_speed();
    }

    // Check ground speed and time delay
    if (((current_speed > g.takeoff_throttle_min_speed || is_zero(g.takeoff_throttle_min_speed))) &&
        ((now - takeoff_state.last_tkoff_arm_time) >= wait_time_ms))
    {
        gcs().send_text(MAV_SEVERITY_INFO, "Triggered AUTO. GPS speed = %.1f", (double)current_speed);
...
bool Plane::check_takeoff_timeout(void)
{
    if (takeoff_state.start_time_ms != 0 && g2.takeoff_timeout > 0)
    {
        ...
        {
            current_speed = AP::gps().ground_speed();
        }
        const float takeoff_min_ground_speed = 4;
        if (current_speed >= takeoff_min_ground_speed)
        {
        ...
        else
        {
            ...
            {
                gcs().send_text(MAV_SEVERITY_INFO, "Takeoff timeout: %.1f m/s speed < 4m/s", current_speed);
